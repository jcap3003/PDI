...
class Camera():
    ...
    def process(self,img):
        ...
        m_roi = mask[int(16*mask.shape[0]/30):int(17*mask.shape[0]/30)].copy()
        u_roi = mask[int(2*mask.shape[0]/30):int(3*mask.shape[0]/30)].copy()
...
#                                    middle centroid
        ...
        if(len(whites)>0.01*m_roi.shape[0]*m_roi.shape[1]):
            unormalized_centroids = np.mean(whites.T, axis = 1)
            cv2.circle(mask, (int(unormalized_centroids[1]),int(unormalized_centroids[0])+ int(16*mask.shape[0]/30)), 3, (255,0,0))
...
#                                    upper centroid
        ...
        if(len(whites)>0.01*u_roi.shape[0]*u_roi.shape[1]):
            unormalized_centroids = np.mean(whites.T, axis = 1)
            cv2.circle(mask, (int(unormalized_centroids[1]),int(unormalized_centroids[0])+ int(2*mask.shape[0]/30)), 3, (0,255,0))
...
#         self.I = 0
    def control_pid(self, d_cent, last_d_cent, dt):
        P = self.kp*d_cent
        self.I += self.ki*self.F1*(d_cent)
        D = self.kd*((d_cent)-(last_d_cent))/(self.F1)
        PID = P+self.I+D
        self.robot.set_motors(self.F1*(2+PID*8), self.F1*(2-PID*8))
    def control_pid_2(self, d_cent, last_d_cent, u_cent, last_u_cent, dt):
        P = self.kp*1.2*d_cent
        self.I += self.ki*self.F2*(d_cent)
        D = self.kd*((d_cent)-(last_d_cent))/self.F2
        PID = P+self.I+D
        
        
        P2 = self.kp2*(u_cent)
        self.I2 += self.ki2*(u_cent)*self.F2
        D2 = self.kd2*((u_cent)-(last_u_cent))/self.F2
        PID2 = P2+self.I2+D2
        #fast?

        
        self.vfactor = 2.5*sigmoid(1-max(u_cent,d_cent))
        TPID = 0.7*PID+0.3*PID2
        
        v_left = self.F2*(2+TPID*6+2*self.vfactor)
        v_right = self.F2*(2-TPID*6+2*self.v_factor) 
    ...
    def control_pid_3(self, d_cent, last_d_cent, m_cent, last_m_cent,u_cent, last_u_cent, dt):
        ...
        P2 = self.kp2*(m_cent)
        self.I2 += self.ki2*(m_cent)*self.F2
        D2 = self.kd2*atanh((m_cent)-(last_m_cent))/self.F2
        PID2 = P2+self.I2+D2
        
        P3 = self.kp3*(u_cent)
        self.I3 += self.ki3*(u_cent)*self.F2
        D3 = self.kd3*atanh((u_cent)-(last_u_cent))/self.F2
        PID3 = P3+self.I3+D3
        self.vfactor = 2.5*sigmoid(1-mean(d_cent, mean(u_cent,m_cent)))
